...
namespace FlatBufferData.Build
{
    public class FBSBuilder
    {
        ...
        /// 文件表
        /// </summary>
        private Dictionary<string, Document> path2document = new Dictionary<string, Document>();
        ...
        public FBSBuilder(string projectName, string[] fileList, ErrorReport report)
        {
        ...
        /// 构建
        /// </summary>
        public Document Build(string path, string text)
        {
        ...
        /// <returns></returns>
        public Document FindDocument(string currPath, string includePath)
        {
            if (!Path.IsPathRooted(includePath))
            {
                var path = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(currPath), includePath));
                if (path2document.ContainsKey(path))
                    return path2document[path];
        ...
        public Document ParseDocument(string path, string text)
        {
            if (path2document.ContainsKey(path))
                path2document.Remove(path);

            var document = new Document(path);

            path2document.Add(path, document);
            ...
            foreach (var context in schema.@namespace())
            {
                if (document.NameSpace == null)
                {
                    ...
                    if (idents.Length > 0)
                    {
                        ...
                        document.NameSpace = string.Join(".", tokens);
                    }
                    else
                        ReportError(document, "错误的 namespace 声明!", context);
                }
                else
                    ReportError(document, "namespace 重复声明!", context);
            }
            //file_extension
            foreach (var context in schema.fileExtension())
            {
                if (document.FileExtension == null)
                {
                    if (context.val == null || context.val.StartIndex == -1)
                        ReportError(document, "错误的file_extension声明。", context);
                    else if (string.IsNullOrEmpty(context.val.Text.Trim('"')))
                        ReportError(document, "file_extension声明的扩展名不能为空。", context.val);
                    else
                        document.FileExtension = context.val.Text.Trim('"');
                }
                else
                    ReportError(document, "file_extension 重复声明。", context);
            }
            //file_identifier
            foreach (var context in schema.fileIdentifier())
            {
                if (document.FileIdentifier == null)
                {
                    if (context.val == null || context.val.StartIndex == -1)
                        ReportError(document, "错误的file_identifier声明。", context);
                    else if (string.IsNullOrEmpty(context.val.Text.Trim('"')))
                        ReportError(document, "错误的file_identifier声明声明的标识符不能为空。", context.val);
                    else
                        document.FileIdentifier = context.val.Text.Trim('"');
                }
                else
                    ReportError(document, "错误的file_identifier声明 重复声明。", context);
            }
            //table
            foreach (var context in schema.table())
            {
                ...
                data.Metas = ParseMetaDatas(document, context.metaList);
                ...
                foreach (var fieldContext in context.tableField())
                {
                    ...
                    field.DefaultValue = ParseDefaultValue(document, field.Type, field.IsArray, fieldContext.fieldValue);
                    field.Metas = ParseMetaDatas(document, fieldContext.metaList);
                    ...
                    {
                        ReportError(document, "名称不能为空。", fieldContext);
                        continue;
                    }
                    else if (fieldNameSet.Contains(field.Name))
                    {
                        ReportError(document, "重复的名称。", fieldContext.fieldName);
                        continue;
                    }
                    else if (string.IsNullOrEmpty(field.Type))
                    {
                        ReportError(document, "类型不能为空。", fieldContext);
                        continue;
                    }
            ...
            foreach (var context in schema.@struct())
            {
                ...
                foreach (var fieldContext in context.structField())
                {
                    ...
                    {
                        ReportError(document, "struct字段的类型不能是数组。", fieldContext.arrayType != null ? fieldContext.arrayType as ParserRuleContext : fieldContext.fieldType as ParserRuleContext);
                        continue;
                    }

                    if (fieldContext.fieldValue != null)
                        ReportError(document, "struct字段目前不支持默认值。", fieldContext.fieldValue);
            ...
            foreach (var context in schema.@enum())
            {
                ...
                foreach (var fieldContext in context.enumField())
                {
                    ...
                    {
                        ReportError(document, "重复的ID。", fieldContext.fieldValue);
                        continue;
                    }
                    else if (fieldNameSet.Contains(field.Name))
                    {
                ...
                if (context.baseType != null)
                {
                    ...
                    if (IsNativeType(name) && !"float".Equals(name) && !"float32".Equals(name) && !"double".Equals(name) && !"float64".Equals(name) && !"string".Equals(name))
                        ...
                        ReportError(document, "enum的基类型必须是整数类型.", context.baseType);
                }
            ...
            foreach (var context in schema.rpc())
            {
                ...
                document.Rpcs.Add(data);
                data2context.Add(data, context);
            }
            //root_type
            foreach (var context in schema.rootType())
            {
                if (document.RootTable == null)
                {
                    if (context.val == null || context.val.StartIndex == -1)
                        ReportError(document, "错误的 root_type 声明。", context);
                    else
                    {
                        var tabName = context.val.Text;
                        if (string.IsNullOrEmpty(tabName))
                            ReportError(document, "错误的 root_type 声明。", context.val);
                        else
                        {
                            foreach (var item in document.Tables) { if (item.Name.Equals(tabName)) { document.RootTable = item; break; } }
                            if (document.RootTable == null)
                                ReportError(document, string.Format("主表定义 ({0}) 未找到!", context.val.Text), context.val);
                        }
                    }
                }
                else
                    ReportError(document, "root_type 重复声明!", context);
            }
            //include
            var includeFBSs = new List<Document>();
            foreach (var context in schema.include())
            {
                if (context.val == null || context.val.StartIndex == -1)
                {
                    ReportError(document, "无效的include。", context);
                    continue;
                }
                var url = context.val.Text.Trim('"');
                if (string.IsNullOrEmpty(url))
                {
                    ReportError(document, "无效的include。", context.val);
                    continue;
                }
                var fbs = FindDocument(document.Path, url);
                if (fbs == null)
                {
                    ReportError(document, string.Format("{0} 未找到。", url), context.val);
                    continue;
                }

                includeFBSs.Add(fbs);
            }
            document.Includes = includeFBSs.ToArray();

            //check
            CheckDefined(document, data2context);

            //attributes
            ParseAttributes(document, data2context);
        ...
        /// <returns></returns>
        private object ParseDefaultValue(Document document, string fieldType, bool isArray, ScalarValueContext context)
        {
            ...
            if (isArray)
            {
                ReportError(document, "列表类型不支持默认值。", context);
                return null;
            }
            else if (fieldType.Equals("bool"))
            {
                ...
                if (bool.TryParse(fieldValue, out boolValue))
                    ...
                    ReportError(document, string.Format("默认值 {0} 不是一个有效的 {1}。", fieldValue, fieldType), context);
            }
            ...
            {
                ReportError(document, string.Format("目前 {0} 不支持默认值。", fieldType), context);
            }
            return null;
        }
        ...
        /// <returns></returns>
        private List<Meta> ParseMetaDatas(Document document, FlatbufferParser.MetadataContext metaList)
        {
            ...
            if (metaList != null)
            {
                foreach (var meta in metaList.metadataField())
                {
                    ...
                    {
                        ReportError(document, "元数据键不能为空", meta);
                        continue;
                    }
        ...
        private void CheckDefined(Document document, Dictionary<object, object> data2context)
        {
            var allDefined = new Dictionary<string, List<object>>();

            //查找能访问到的的所有定义
            var flaged = new HashSet<Document>();
            var queue = new List<Document>();
            queue.Add(document);
            while (queue.Count > 0)
            {
            ...
            //确定名称空间
            var ns = !string.IsNullOrEmpty(document.NameSpace) ? document.NameSpace + "." : "";

            //检查 Table
            foreach (var data in document.Tables)
            {
                //table name
                if (string.IsNullOrEmpty(data.Name)) continue;
                var key = ns + data.Name;
                if (allDefined.ContainsKey(key) && allDefined[key].Count > 1)
                    ReportError(document, "名称重复定义。", (data2context[data] as TableContext).name);
                ...
                foreach (var field in data.Fields)
                {
                    ...
                    if (allDefined.ContainsKey(typeName))
                    {
                        if (allDefined[typeName].Count > 1)
                            ReportError(document, string.Format("找到多个名称为 {0} 的定义。", field.Type), locator);
                        else if (allDefined[typeName].Count == 1)
                        {
                            ...
                            if (first is Table || first is Struct || first is Enum || first is Union)
                                ...
                                ReportError(document, "table字段的类型不能是rpc。", locator);
                        }
                    }
                    else
                        ReportError(document, string.Format("没有找到 {0} 的定义。", field.Type), locator);
                }
            }
            //检查 Struct
            foreach (var data in document.Structs)
            {
                //Struct 名称
                if (string.IsNullOrEmpty(data.Name)) continue;
                var key = ns + data.Name;
                if (allDefined.ContainsKey(key) && allDefined[key].Count > 1)
                    ReportError(document, "名称重复定义。", (data2context[data] as StructContext).name);
                ...
                foreach (var field in data.Fields)
                {
                    ...
                    if ("string".Equals(field.Type))
                        ReportError(document, "struct字段的类型只能是bool, number, enum, struct。", locator);
            ...
            //检查 Enum
            foreach (var data in document.Enums)
            {
                //Enum 名称
                if (string.IsNullOrEmpty(data.Name)) continue;
                var key = ns + data.Name;
                if (allDefined.ContainsKey(key) && allDefined[key].Count > 1)
                    ReportError(document, "名称重复定义。", (data2context[data] as EnumContext).name);

                //Enum 字段
                //null
            }
            //检查 Union
            foreach (var data in document.Unions)
            {
                //Union 名称
                if (string.IsNullOrEmpty(data.Name)) continue;
                var key = ns + data.Name;
                if (allDefined.ContainsKey(key) && allDefined[key].Count > 1)
                    ReportError(document, "名称重复定义。", (data2context[data] as UnionContext).name);
                ...
                foreach (var field in data.Fields)
                {
                    ...
                    {
                        ReportError(document, "union字段的类型只能是string, struct, table。", locator);
                    }
                    else
                    {
                        var typeName = field.Type.IndexOf(".") != -1 ? field.Type : ns + field.Type;
            ...
            //检查 Rpc
            foreach (var data in document.Rpcs)
            {
                //Rpc 名称
                if (string.IsNullOrEmpty(data.Name)) continue;
                var key = ns + data.Name;
                if (allDefined.ContainsKey(key) && allDefined[key].Count > 1)
                    ReportError(document, "名称重复定义。", (data2context[data] as RpcContext).name);
                ...
                foreach (var field in data.Fields)
                {
                    if (!string.IsNullOrEmpty(field.Param) && !IsNativeType(field.Param))
                    {
                        ...
                        {
                            if (allDefined[typeName].Count > 1)
                                ReportError(document, string.Format("找到多个名称为 {0} 的定义。", field.Param), (data2context[field] as RpcFieldContext).fieldParam);
                            else if (allDefined[typeName].Count == 1)
                            {
                                var first = allDefined[typeName][0];
                                if (first is Struct || first is Table || first is Enum || first is Union)
                                    ...
                                    ReportError(document, "rpc字段的参数类型不能是rpc", (data2context[field] as RpcFieldContext).fieldParam);
                            }
                        }
                        else
                            ReportError(document, string.Format("没有找到 {0} 的定义。", field.Param), (data2context[field] as RpcFieldContext).fieldParam);
                    }
                    if (!string.IsNullOrEmpty(field.Return) && !IsNativeType(field.Return))
                    {
                        ...
                        {
                            if (allDefined[typeName].Count > 1)
                                ReportError(document, string.Format("找到多个名称为 {0} 的定义。", field.Return), (data2context[field] as RpcFieldContext).fieldReturn);
                            else if (allDefined[typeName].Count == 1)
                            {
                                var first = allDefined[typeName][0];
                                if (first is Struct || first is Table || first is Enum || first is Union)
                                    ...
                                    ReportError(document, "rpc字段的返回类型不能是rpc", (data2context[field] as RpcFieldContext).fieldReturn);
                            }
                        }
                        else
                            ReportError(document, string.Format("没有找到 {0} 的定义。", field.Return), (data2context[field] as RpcFieldContext).fieldReturn);
                    }
                }
            }
        }
        ...
        /// 解析特性
        /// </summary>
        private void ParseAttributes(Document document, Dictionary<object, object> data2context)
        {
            foreach (var data in document.Tables)
            {
                ParseAttribute(document, (data2context[data] as TableContext).attr(), data, data.Attributes);
                foreach (var field in data.Fields)
                    ParseAttribute(document, (data2context[field] as TableFieldContext).attr(), field, field.Attributes);
            }
            foreach (var data in document.Structs)
            {
                ParseAttribute(document, (data2context[data] as StructContext).attr(), data, data.Attributes);
                foreach (var field in data.Fields)
                    ParseAttribute(document, (data2context[field] as StructFieldContext).attr(), field, field.Attributes);
            }
            foreach (var data in document.Enums)
            {
                ParseAttribute(document, (data2context[data] as EnumContext).attr(), data, data.Attributes);
                foreach (var field in data.Fields)
                    ParseAttribute(document, (data2context[field] as EnumFieldContext).attr(), field, field.Attributes);
            }
            foreach (var data in document.Unions)
            {
                ParseAttribute(document, (data2context[data] as UnionContext).attr(), data, data.Attributes);
                foreach (var field in data.Fields)
                    ParseAttribute(document, (data2context[field] as UnionFieldContext).attr(), field, field.Attributes);
            }
            foreach (var data in document.Rpcs)
            {
                ParseAttribute(document, (data2context[data] as RpcContext).attr(), data, data.Attributes);
                foreach (var field in data.Fields)
                    ParseAttribute(document, (data2context[field] as RpcFieldContext).attr(), field, field.Attributes);
            }
        }
        ...
        /// 解析特性
        /// </summary>
        private void ParseAttribute(Document document, AttrContext[] contexts, object owner, AttributeTable attributes)
        {
            foreach (var context in contexts)
            {
                ...
                {
                    ReportError(document, "特性名称不能为空", context);
                    continue;
                }
                ...
                {
                    ReportError(document, "不支持的特性", context.key);
                    return;
                }
                ...
                foreach (var attributeMeta in attributeType.GetCustomAttributes())
                {
                    if (attributeMeta is AllowTarget)
                        allow = OnAllowTarget(document, attributeType, context, owner, attributes, attributeMeta as AllowTarget) && allow;
                    else if (attributeMeta is AllowMultiple)
                        allow = OnAllowMultiple(document, attributeType, context, owner, attributes, attributeMeta as AllowMultiple) && allow;
                    else if (attributeMeta is ConflictFlag)
                        allow = OnConflictFlag(document, attributeType, context, owner, attributes, attributeMeta as ConflictFlag) && allow;
                    else if (attributeMeta is RequiredFlag)
                        allow = OnRequiredFlag(document, attributeType, context, owner, attributes, attributeMeta as RequiredFlag) && allow;
                    else if (attributeMeta is RequiredArrayField)
                        allow = OnRequiredArrayField(document, attributeType, context, owner, attributes, attributeMeta as RequiredArrayField) && allow;
                    else if (attributeMeta is RequiredFieldType)
                        allow = OnRequiredFieldType(document, attributeType, context, owner, attributes, attributeMeta as RequiredFieldType) && allow;
                }

                if (allow)
                    CreateAttribute(document, attributeType, context, attributes);
            }
        }
        ...
        /// <returns></returns>
        private bool OnAllowTarget(Document document, System.Type attributeType, AttrContext context, object owner, AttributeTable attributes, AllowTarget arg)
        {
            if (arg.AllowTargets != TargetTypeID.ANY)
            {
                ...
                if (value == 0)
                {
                    ...
                    ReportError(document, string.Format("{0} 只能应用到 {1}.", attributeType.Name, string.Join(",", infos)), context.key);
                    return false;
                }
            }
        ...
        /// <param name="arg"></param>
        /// <returns></returns>
        private bool OnAllowMultiple(Document document, System.Type attributeType, AttrContext context, object owner, AttributeTable attributes, AllowMultiple arg)
        {
            if (arg.Allow == false && attributes.HasAttribute(attributeType))
            {
                ReportError(document, string.Format("{0}不能多次应用到同一个对象.", attributeType.Name), context.key);
                return false;
            }
        ...
        /// <param name="arg"></param>
        /// <returns></returns>
        private bool OnConflictFlag(Document document, System.Type attributeType, AttrContext context, object owner, AttributeTable attributes, ConflictFlag arg)
        {
            foreach (var type in arg.ConflictTypes)
            {
                if (attributes.HasAttribute(type))
                {
                    ReportError(document, string.Format("{0} 不能和 {1} 应用到同一个对象。", attributeType.Name, type.Name), context.key);
                }
            }
            return true;
        }
        ...
        /// <param name="arg"></param>
        /// <returns></returns>
        private bool OnRequiredFlag(Document document, System.Type attributeType, AttrContext context, object owner, AttributeTable attributes, RequiredFlag arg)
        {
            bool finded = false;
            ...
            if (!finded)
            {
                ...
                ReportError(document, string.Format("{0} 需要和这些标记一起使用（{1}）。", attributeType.Name, string.Join(",", names)), context.key);
            }
            return true;
        }
        ...
        /// <param name="arg"></param>
        /// <returns></returns>
        private bool OnRequiredArrayField(Document document, System.Type attributeType, AttrContext context, object owner, AttributeTable attributes, RequiredArrayField arg)
        {
            var field = owner as TableField;
            if (field == null || !field.IsArray)
            {
                ReportError(document, string.Format("{0} 只能应用到数组型 TableField！", attributeType.Name), context.key);
                return false;
            }
            return true;
        }
        ...
        /// <param name="arg"></param>
        /// <returns></returns>
        private bool OnRequiredFieldType(Document document, System.Type attributeType, AttrContext context, object owner, AttributeTable attributes, RequiredFieldType arg)
        {
            if (arg.FieldType != FieldTypeID.ANY)
            {
                ...
                if ((arg.FieldType & typeID) == 0)
                {
                    ...
                    ReportError(document, string.Format("{0} 只能应用到类型为 {1} 的 TableField 和 StructField ！", attributeType.Name, string.Join(",", infos)), context.key);
                    return false;
                }
            }
        ...
        /// <param name="owner"></param>
        /// <param name="attributes"></param>
        private void CreateAttribute(Document document, System.Type attributeType, AttrContext context, AttributeTable attributes)
        {
            ...
            if (constructors.Length > 0)
            {
                ...
                ReportError(document, string.Format("格式错误, 应该是 [ {0}{1} ]。", attributeType.Name, errorInfo.Count > 0 ? " ( " + string.Join(" , ", errorInfo) + " )" : ""), context);
            }
        }
        ...
        private void ReportError(Document document, string text, IToken token)
        {
            ReportError(document, text, token.Line, token.Column, token.StartIndex, token.StopIndex - token.StartIndex + 1);
        }
        private void ReportError(Document document, string text, ParserRuleContext context)
        {
            ReportError(document, text, context.Start.Line, context.Start.Column, context.Start.StartIndex, context.Stop.StopIndex - context.Start.StartIndex + 1);
        }
        private void ReportError(Document document, string text, int line, int column, int begin, int count)
        {
            report?.Invoke(projectName, document.Path, text, line, column, begin, count);
        }
        private void ReportWarning(Document document, string text, IToken token)
        {
            ReportError(document, text, token.Line, token.Column, token.StartIndex, token.StopIndex - token.StartIndex + 1);
        }
        private void ReportWarning(Document document, string text, ParserRuleContext context)
        {
            ReportError(document, text, context.Start.Line, context.Start.Column, context.Start.StartIndex, context.Stop.StopIndex - context.Start.StartIndex + 1);
        }
        private void ReportWarning(Document document, string text, int line, int column, int begin, int count)
        {
            report?.Invoke(projectName, document.Path, text, line, column, begin, count);
        }
